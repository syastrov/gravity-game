...
class Chunk:
  ...
  def update(self):
    ...
    if dist < planet.size + self.size:
      global score
      explosions = True
      if self.bad and explosions:
        for c in chunks:
          if c.attach:
            c.acceleration = (100.0, 100.0)

            # Explode the chunks in random directions at random speeds
            direction = random.uniform(0, 2*pi)
            speed = random.uniform(100, 400)
            # Give them a head start (in seconds) to get away from the cursor so it doesn't get attached again
            headStart = 0.5
            c.velocity = speed * cos(direction), speed * sin(direction)
            c.pos = c.pos[0] + c.velocity[0] * elapsed, c.pos[1] + c.velocity[1] * headStart
    ...
    accel = (G * planet.mass * self.mass) / dist**2
...
def startGame(numChunks):
  ...
  for i in range(numChunks):
    color = [c + random.randint(-128, 127) for c in grey]
    chunks.append(Chunk(color, randomPosition(), 8 + random.randint(-3, 30), random.random() < 0.1))
